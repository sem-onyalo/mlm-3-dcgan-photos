...
from data import generateRealSamples, generateFakeSamples, generateLatentPoints
...
def train(discriminator: Sequential, generator: Sequential, gan: Sequential, dataset, latentDim, epochNum=200, batchNum=128, evalFreq=10):
    batchesPerEpoch = int(dataset.shape[0] / batchNum)
    halfBatch = int(batchNum / 2)
    ...
    for i in range(epochNum):
        for j in range(batchesPerEpoch):
            ...
            xGan = generateLatentPoints(latentDim, batchNum)
            yGan = ones((batchNum, 1))
